...
El problema del \textbf{Bandido Multibrazo} (o \textit{k-armed bandit}) modela una situación 
de toma de decisiones en presencia de incertidumbre. En este escenario, 
un agente debe elegir repetidas veces entre las $k$ palancas o ``brazos'' de una máquina, donde cada uno ofrece recompensas 
que siguen una distribución de probabilidad desconocida para el agente. 
De esta manera, el objetivo es maximizar la recompensa acumulada a lo largo de un horizonte temporal $T$.

Este problema es un caso de estudio fundamental en el campo del Aprendizaje por Refuerzo, 
por ser un ejemplo básico del \textbf{dilema de la Exploración-Explotación}.
...
El estudio del bandido de $k$-brazos es importante, tanto como base teórica para comprender 
problemas más complejos, como por su aplicabilidad en múltiples y variados problemas del mundo real.
...
en plataformas de streaming, o la \textbf{asignación de promociones} en aplicaciones de compra online.
 
La motivación principal de este trabajo es comprender cómo diferentes algoritmos y 
distribuciones de recompensa afectan al rendimiento de los agentes en este entorno.



\subsection{Objetivos del informe}
El objetivo principal de este informe técnico es realizar un estudio comparativo de 
...
\begin{itemize}
    \item \textbf{Métodos Voraces (Greedy) y $\epsilon$-Greedy:} Son los enfoques más simples, 
    donde el agente alterna entre explotar el conocimiento actual con probabilidad $1-\epsilon$ 
    y explorar aleatoriamente con probabilidad $\epsilon$. Aunque efectivos, no consideran 
    la varianza de las estimaciones.
    \item \textbf{Métodos de Intervalo de Confianza (UCB):} Basados en el principio de 
    ``optimismo ante la incertidumbre'', estos algoritmos (como UCB1) seleccionan 
    acciones basándose en el límite superior de un intervalo de confianza, favoreciendo
     acciones poco exploradas de manera determinista.
    \item \textbf{Métodos de Gradiente y Softmax:} A diferencia de los anteriores, 
    que estiman valores de acción, 
...
El algoritmo UCB1 selecciona acciones maximizando una cota superior de confianza, expresada como suma del valor estimado $Q_t(a)$ y un término de exploración:
\begin{equation*}
...
Como extensión del anterior, se ha implementado el algoritmo UCB2, el cual introduce una estructura de \textbf{épocas} para mejorar la eficiencia de la exploración. A diferencia de UCB1, que reevalúa la decisión en cada paso $t$, UCB2 selecciona un brazo y lo mantiene activo durante un intervalo de tiempo:
\begin{equation*}
    \tau(r) = \lceil (1+\alpha)^r \rceil
\end{equation*}
donde $\alpha > 0$ es un parámetro que regula el incremento de las épocas. Esta estrategia reduce la frecuencia de cambio entre brazos y permite obtener teóricamente mejores cotas de rechazo asintótico.
...
\subsection{Algoritmos UCB (Upper Confidence Bound)}

\subsubsection{Descripción y Justificación}
El algoritmo UCB1 selecciona acciones maximizando una cota superior del intervalo de confianza. 
La inclusión del algoritmo UCB1 y su variante UCB2 en este estudio comparativo se justifica por 
dos factores críticos:

\begin{itemize}
    \item \textbf{Garantías de Convergencia:} Auer et al. (2002) \cite{auer2002finitetime}  demostraron que UCB1 garantiza una cota 
    ...
    pero decrece con las visitas locales $N_t(a)$, forzando a explorar brazos poco 
    visitados inicialmente y a converger hacia la explotación del brazo óptimo conforme se reduce la incertidumbre.
\end{itemize}
...
En este capítulo se detallan las pruebas realizadas para evaluar los 
algoritmos implementados. El objetivo es analizar cómo estos se comportan 
bajo distintas distribuciones de recompensa y configuraciones de hiperparámetros.
...
\begin{itemize}
    ...
    Este esquema combina una exploración intensiva inicial con una explotación a largo plazo, logrando el mejor compromiso global entre rapidez de aprendizaje y rendimiento.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{resources/egreedy_reward_normal_sigma1.png}
    \caption{Recompensa promedio en función del número de pasos para $\epsilon$-Greedy bajo distribución Normal con $\sigma=1$.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{resources/egreedy_variance_comparison.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{resources/egreedy_bernoulli_binomial.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/softmax_histograma.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/softmax_histograma2.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/softmax_regret.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/softmax_bernoulli.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/softmax_exploracion_inicial.png}
...
\begin{itemize}
    ...
    se vuelve más voraz, como se puede observar en la Figura \ref{fig:ucb1_recompensas}.
    \item \textbf{UCB2}: También es un algoritmo muy eficaz, alcanzando rendimientos similares a UCB1. Al trabajar con 
    épocas, los agentes presentan un comportamiento oscilatorio entre explotación del brazo óptimo y exploración de 
    brazos subóptimos, controlado por el parámetro $\alpha$. El valor $\alpha=0.1$ mostró el mejor rendimiento en 
    nuestros experimentos, seguido por $\alpha=0.001$. Sin embargo, para el valor intermedio de $\alpha=0.01$ las 
    oscilaciones mencionadas tenían un periodo de orden de magnitud similar al horizonte temporal del experimento, 
    lo que causaba una caída considerable en el rendimiento en la mitad de cada ejecución, como se puede observar 
    en la Figura \ref{fig:ucb2_recompensas}. 
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/ucb1_recompensas.png}
\caption{Evolución de las recompensas para el algoritmo UCB1 con distribución Normal para distintos parámetros.}
\label{fig:ucb1_recompensas}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/ucb2_recompensas.png}
\caption{Evolución de las recompensas para el algoritmo UCB2 con distribución Normal para distintos parámetros.}
\label{fig:ucb2_recompensas}
\end{figure}

También se ha comprobado empíricamente que el regret acumulado crece de manera logarítmica para ambas variantes (aunque 
para algunos valores de $\alpha$ en UCB2 no se aprecia claramente a esta escala), lo que es consistente con las garantías 
teóricas de estos algoritmos. Esto puede verse en la Figura \ref{fig:ucb_regret} para el experimento con distribución Normal, 
pero es un comportamiento que se repite para las otras configuraciones.

\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/ucb_regret.png}
\caption{Evolución del regret acumulado para los algoritmos UCB1 y UCB2 con distribución Normal.}
\label{fig:ucb_regret}
\end{figure}

Comparando los resultados obtenidos entre distintos experimentos, observamos que en el experimento Binomial fueron muy similares 
a los obtenidos en el Normal, mientras que con Bernoulli las recompensas acumuladas son más bajas y los regrets suben considerablemente, incluso cuadruplicándose en algunos casos,
por la mayor dificultad del problema. El regret acumulado para UCB1 y UCB2 con distribución de Bernoulli se muestra en la Figura \ref{fig:ucb_bernoulli_regret}.
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{resources/ucb_bernoulli_regret.png}
\caption{Evolución del regret acumulado para los algoritmos UCB1 y UCB2 con distribución de Bernoulli.}
\label{fig:ucb_bernoulli_regret}
\end{figure}

\textbf{Resultados obtenidos:}
\begin{itemize}
    \item Ambas variantes de UCB demostraron un rendimiento superior en términos de recompensa promedio y porcentaje de selección del brazo óptimo, superando a las familias $\epsilon$-Greedy y Softmax.
    \item La elección adecuada $\alpha$ para UCB2, es crucial para obtener un buen rendimiento con este algoritmo. A pesar de que sus cotas asintóticas son las mejores, con las duraciones de experimento que hemos usado es un 
    algoritmo delicado y muy sensible a la configuración de este hiperparámetro, lo que puede limitar su aplicabilidad práctica.
    \item UCB1 mostró un rendimiento robusto y consistente en todas las configuraciones, con variaciones suaves al modificar el parámetro $C$.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{resources/global_reward_comparison.png}
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{resources/global_regret_comparison.png}
    \caption{Regret acumulado en el experimento comparativo global.}
    \label{fig:global_regret}
    \end{figure}


    \item \textbf{Conclusión:} En este experimento concreto, UCB1 se posiciona como la estrategia más eficiente y robusta, combinando rápida convergencia, máxima selección óptima y bajo regret. UCB2 ofrece resultados prácticamente equivalentes, aunque algo más inestables. Softmax y $\epsilon$-Greedy con $\epsilon$ pequeño son competitivos en recompensa promedio, pero menos eficientes en términos de identificación del brazo óptimo. 
    El esquema con decaimiento resulta más sensible al horizonte temporal considerado, pues necesitaría como 
    ya hemos comentado un horizonte mayor para superar su coste inicial de exploración y alcanzar el mismo 
    rendimiento que los otros algoritmos.
\end{itemize}
...
A partir del análisis experimental realizado sobre el problema del bandido de $k$-brazos bajo distintas distribuciones de recompensa, se extraen las siguientes conclusiones fundamentales:

\begin{itemize}
    \item \textbf{Sensibilidad de los métodos básicos:} El rendimiento de la familia $\epsilon$-Greedy depende 
    críticamente del nivel de ruido del entorno y del ajuste de $\epsilon$. Mientras que la explotación pura 
    ($\epsilon=0$) es eficiente en entornos de muy baja varianza, se obtienen muy malos resultados cuando hay mucha varianza. 
    La introducción de un $\epsilon$-decaimiento obtiene los resultados más robustos dentro de esta familia, aunque 
    la gran exploración con la que empieza en los primeros episodios penaliza la recompensa en horizontes temporales cortos.
    
    \item \textbf{Dependencia paramétrica y estancamiento en Softmax:} El algoritmo Softmax es muy sensible a 
    la selección de su temperatura $\tau$. Temperaturas bajas ($\tau=0.1$) provocan una convergencia prematura hacia 
    óptimos locales debido a ser prácticamente voraz. Este defecto solo logra mitigarse de forma natural en entornos
     muy penalizadores (como la distribución de Bernoulli) o forzando explícitamente una pasada de 
    exploración inicial en el algoritmo. 
    
    \item \textbf{Superioridad de los métodos basados en incertidumbre (UCB):} Las estrategias UCB1 y 
    UCB2 han demostrado ser las mejores para este problema. Al balancear automáticamente la 
    exploración y la explotación sin depender de decaimientos manuales, logran estabilizarse 
    rápidamente. En el análisis global comparativo, UCB1 se posicionó como la estrategia óptima, 
    rozando un 99\% de precisión en la selección del brazo correcto y minimizando el \textit{regret} 
    acumulado con un crecimiento logarítmico, lo cual confirma en la práctica las garantías 
    teóricas establecidas en la literatura.
\end{itemize}



En definitiva, se concluye que, para entornos estacionarios con incertidumbre, los algoritmos basados 
en la cota superior de confianza (especialmente UCB1) superan sistemáticamente a los métodos basados 
en probabilidad pura o exploración aleatoria constante.
